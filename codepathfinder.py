...
class TestPathFinder:
    ...
    def calculate_test_path_complexity(self, path):
         path_complexity = 0  # Initialize complexity count
         for vertex_id in path:
          vertex_description = self.vertices[vertex_id].vertex_description

        # Check for decision points and loops in the vertex description
          if ('If condition' in vertex_description or 
            'Elif condition' in vertex_description or  # Add elif condition
            'For loop' in vertex_description or 
            'While condition' in vertex_description or 
            'Try block' in vertex_description or  # Add try-except for exception handling
            'Except block' in vertex_description):  # Exception handler block
            path_complexity += 1

            return path_complexity  # Return the total complexity
    ...
    def visualize_cfg(self):
     ...
     for node_id, label in self.mynodes:
        dot.node(
            ...
            shape="plaintext",
            length='2.5',
            width='2.5',  # Adjusted size for better visualization
            fixedsize="true",
            style="filled",
            fillcolor="lightgrey",
            fontsize="12",
     ...
     dot.render("cfg", format="png", cleanup=True)  # Generates a PNG file
     dot.view("cfg")  # Opens the generated graph

def main():
    file_path = r"C:/Users/HANAN/OneDrive/Desktop/Dsa project/example_program.py"
